...
function updateLeaderboard() {
    ...
    const scoresList = leaderboard
        .sort((a, b) => b.score - a.score)
        .slice(0, 5)
        .map((entry, index) => `${entry.name}: ${entry.score}`)
        .join('</li><li>');
    
    if (startScores) {
        startScores.innerHTML = `<li>${scoresList}</li>`;
    }
    if (topScores) {
        topScores.innerHTML = `<li>${scoresList}</li>`;
    }
}
...
function updateEnvironment() {
    ...
    if (scene.userData.sunGroup) {
        ...
        sunGroup.rotation.y += 0.0005; // Slowed down rotation
        sunGroup.rotation.z = Math.sin(Date.now() * 0.0003) * 0.05; // Reduced wobble
    }
